...
function transform(data) {
  ...
  // Sélectionner et renommer les colonnes : Code commune → code_postal, Conso totale (MWh) → consommation_totale, Conso moyenne (MWh) → consommation_moyenne, FILIERE → gaz, Année → annee_consommation
  const transformedData = data.map((row) => ({
    code_postal: row['Code commune'] ? row['Code commune'].trim() : null,
